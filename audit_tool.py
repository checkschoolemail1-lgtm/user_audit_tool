...
def analyze_users(users):
    ...
    pct_dormant = sum(1 for u in users if u["last_login_days"] >= DORMANCY_THRESHOLD) / len(users) * 100
    pct_no_mfa = sum(1 for u in users if not u["mfa"]) / len(users) * 100

    # ORG-WIDE FINDINGS
    if pct_admin > ADMIN_RATIO_THRESHOLD:
        org_events.add(f"High admin ratio ({pct_admin:.2f}%)")

    if pct_dormant > DORMANT_RATIO_THRESHOLD:
        org_events.add(f"High dormant ratio ({pct_dormant:.2f}%)")

    if pct_no_mfa > NO_MFA_RATIO_THRESHOLD:
    ...
    for user in users:
        ...
        if user["last_login_days"] >= DORMANCY_THRESHOLD:
        ...
        if user["last_login_days"] >= DORMANCY_THRESHOLD and not user["mfa"]:
            findings.append("Dormant + No MFA")
            score += 7

        if user["role"] == "admin" and user["last_login_days"] >= DORMANCY_THRESHOLD and not user["mfa"]:
...
def generate_summary(report):
    ...
    return {
        ...
        "critical": sum(1 for u in users if u["severity"] == "CRITICAL"),
        "high": sum(1 for u in users if u["severity"] == "HIGH"),
        "medium": sum(1 for u in users if u["severity"] == "MEDIUM"),
        "low": sum(1 for u in users if u["severity"] == "LOW"),
    }
...
def main():
    ...
    filename = input("Enter JSON file with users (default: users.json): ").strip()
    if filename == "":
        filename = "users.json"
